...
void setproll(uint16_t pr) {
  float converted_pr = (float) pr / 1000;
  Kp_roll_angle = converted_pr; //0.5; // default: 0.2;    //Roll P-gain - angle mode
  Serial.print("Kp_roll_angle: ");
  Serial.println(Kp_roll_angle);
}

void setiroll(uint16_t ir) {
  float converted_ir = (float) ir / 1000;
  Ki_roll_angle = converted_ir; // default: 0.3; // Roll I-gain - angle mode
  Serial.print("Ki_roll_angle: ");
  Serial.println(Ki_roll_angle);
}

void setdroll(uint16_t dr) {
  float converted_dr = (float) dr / 1000;
  Kd_roll_angle = converted_dr; // default: 0.05;   //Roll D-gain - angle mode (if using controlANGLE2(), has no effect. Use B_loop_roll)
  Serial.print("Kd_roll_angle: ");
  Serial.println(Kd_roll_angle);
}

void setppitch(uint16_t pp) {
  float converted_pp = (float) pp / 1000;
  Kp_pitch_angle = converted_pp; // default: 0.2;   //Pitch P-gain - angle mode
  Serial.print("Kp_pitch_angle: ");
  Serial.println(Kp_pitch_angle);
}

void setipitch(uint16_t ip) {
  float converted_ip = (float) ip / 1000;
  Ki_pitch_angle = converted_ip; // default: 0.3;   //Pitch I-gain - angle mode
  Serial.print("Ki_pitch_angle: ");
  Serial.println(Ki_pitch_angle);
}

void setdpitch(uint16_t dp) {
  float converted_dp = (float) dp / 1000;
  Kd_pitch_angle = converted_dp; // default: 0.05;  //Pitch D-gain - angle mode (if using controlANGLE2(), has no effect. Use B_loop_pitch)
  Serial.print("Kd_pitch_angle: ");
  Serial.println(Kd_pitch_angle);
}

void setpyaw(uint16_t py) {
  float converted_py = (float) py / 1000;
  Kp_yaw = converted_py;           //Yaw P-gain
  Serial.print("Kp_yaw: ");
  Serial.println(Kp_yaw);
}

void setiyaw(uint16_t iy) {
  float converted_iy = (float) iy / 1000;
  Ki_yaw = converted_iy;           //Yaw I-gain
  Serial.print("Ki_yaw: ");
  Serial.println(Ki_yaw);
}

void setdyaw(uint16_t dy) {
  float converted_dy = (float) dy / 1000;
  Kd_yaw = converted_dy;       //Yaw D-gain (be careful when increasing too high, motors will begin to overheat!)
  Serial.print("Kd_yaw: ");
...
{
#ifdef __LOG__
  Serial.println("Writing Signiture");
#endif
  Serial.println(signiture);

  write_eeprom(35, signiture >> 8);
  write_eeprom(35+1, signiture);
}

uint16_t signiture_read()
{
#ifdef __LOG__
  Serial.println("Reading Signiture");
#endif
  byte byte1 = read_eeprom(35);
  byte byte2 = read_eeprom(35 + 1);
