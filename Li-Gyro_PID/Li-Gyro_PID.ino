...
//Signature to indicate whether PID parameters have been stored or not
#define SIGNITURE 0xAAAA
...
//Variable to read PID parameter from V7RC
uint16_t pidfromV7RC[9]={99999,99999,99999,99999,99999,99999,99999,99999,99999};
uint8_t num_pid = 9;
bool update_pid_parameters = false;
...
void read_pid_from_flash()
{
  ...
  {
    pidfromV7RC[i] = pid_read(i);

#ifdef __LOG__
    Serial.print("pidfromV7RC[" + String(i) + "]: ");
    Serial.println(pidfromV7RC[i]);
#endif
  }

  if (SIGNITURE == signiture_read())
  {
    update_pid_parameters = true;
  }
  else
  {
    update_pid_parameters = false;
  }
}
...
{
  for(int i=0;i<9;i++){
    if (99999 != pidfromV7RC[i])
    {
      switch(i)
      {
        case 0:
          setproll(pidfromV7RC[i]);
          break;
        case 1:
          setiroll(pidfromV7RC[i]);
          break;
        case 2:
          setdroll(pidfromV7RC[i]);
          break;
        case 3:
          setppitch(pidfromV7RC[i]);
          break;
        case 4:
          setipitch(pidfromV7RC[i]);
          break;
        case 5:
          setdpitch(pidfromV7RC[i]);
          break;
        case 6:
          setpyaw(pidfromV7RC[i]);
          break;
        case 7:
          setiyaw(pidfromV7RC[i]);
          break;
        case 8:
          setdyaw(pidfromV7RC[i]);
          break;
        default:
          break;
      }
...
void loop() 
{
  ...
  else
  {
    ...
    if (true == hasCommand)
    {
      ...
      // If PID parameter update is received
      if (true == update_pid_parameters)
      {
        updatePID();
      }
...
bool getRadioCommand()
{
  ...
  if (packetSize) {
    ...
    if(packetBuffer[1]=='I'){
      for(int i=0;i<9;i++){
        data = rxData.substring(i*5+3, i*5+8); 
        Serial.println(data);
        pidfromV7RC[i] = data.toInt();
      }
...
#ifdef __LOG__
    Serial.print(packetBuffer[2]);  /// should be I / V / T / 8 (PID, 2 ch, 4 ch , 8 ch )
    Serial.print(",");
    for(int i=0;i<8;i++){
    ...
    if (true == update_pid_parameters)
    {
      ...
      Serial.print(",");
      for(int i=0;i<9;i++){
        Serial.print(pidfromV7RC[i]);
        Serial.print(",");
      }
      Serial.println(",");
#endif /* __LOG__ */
      pid_write(pidfromV7RC, num_pid);
      signiture_write(SIGNITURE);
    }
    Serial.println();
