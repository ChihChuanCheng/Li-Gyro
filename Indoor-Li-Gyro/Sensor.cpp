...
void genMPU6050Sample() {
  ...
  if (samplingTime <= (millis() - timeCurrent)) {
    ...
    calculateAngle(calibrateddata, false);
  }
}
...
void loopRate(int freq) {
  ...
  */
  ...
  while (invFreq > (checker - timeCurrent)) {
    checker = millis();
  }
}
...
float getDeviceGyX() {
  return angledata.GyX;
}

float getDeviceGyY() {
  return angledata.GyY;
}

float getDeviceGyZ() {
  return angledata.GyZ;
}
...
void printRollPitchYaw() {
  ...
  Serial.print(deviceangle.RX);
  Serial.print(F(" GyroY: "));
  Serial.print(deviceangle.RY);
  Serial.print(F(" GyroZ: "));
  Serial.println(angledata.GyZ);
}
